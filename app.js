...
const DIRECTORY_CATEGORIES = [...new Set(LONDON_DIRECTORY.filter(e=>e.category).map(e=>e.category))].sort();
...
<span class="directory-entry-name">${name}</span>\
${catLabel}\
<span class="directory-entry-location">${loc}</span>\
</div>`;
  }).join('');
